...
static void test_create_string(unit_test_t * tst)
{
  ...
  ASSERT(tst,
         LISP_REFCOUNT(&str) == 1);
  ASSERT_EQ_U(tst,
              lisp_string_length(LISP_AS(&str,
                                         lisp_string_t)), 3u);
  ASSERT_EQ_CSTR(tst,
                 lisp_c_string(LISP_AS(&str,
                                       lisp_string_t)), "abc");
  ASSERT_EQ_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&str, lisp_string_t), "abc"),0);
  ASSERT_LT_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&str, lisp_string_t), "def"),0);
  ASSERT_GT_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&str, lisp_string_t), "012"),0);
  ASSERT_IS_OK(tst,
...
static void test_sprintf(unit_test_t * tst) 
{
  ...
  ASSERT(tst,
         LISP_IS_STRING(&str));
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&str), 1u);
  ASSERT_EQ_CSTR(tst,
                 lisp_c_string(LISP_AS(&str, lisp_string_t)), "1 ff abc");
  ASSERT_EQ_U(tst,
              lisp_string_length(LISP_AS(&str,
                                         lisp_string_t)), 8u);
  ASSERT_IS_OK(tst,
...
static void _helper_lisp_va_sprintf(unit_test_t * tst, const char * fmt, ...)
{
  ...
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&str), 1u);
  ASSERT_EQ_CSTR(tst,
                 lisp_c_string(LISP_AS(&str, lisp_string_t)),
                 "1 ff abc");
  ASSERT_EQ_U(tst,
              lisp_string_length(LISP_AS(&str,
                                         lisp_string_t)), 8u);
...
static void test_lisp_string_cmp_c_string(unit_test_t * tst) 
{
  ...
  lisp_make_string(vm, &str, "abc");
  ASSERT_EQ_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&str, lisp_string_t), "abc"),
              0);
  ASSERT_LT_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&str, lisp_string_t), "xxx"),
              0);
  ASSERT_LT_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&str, lisp_string_t), "abcdef"),
              0);
  ASSERT_GT_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&str, lisp_string_t), "000"),
              0);
  ASSERT_GT_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&str, lisp_string_t), "ab"),
              0);
  ASSERT_IS_OK(tst,
               lisp_unset_object(vm, &str));
  lisp_free_vm(vm);
...
static void test_copy_string(unit_test_t * tst) 
{
  ...
  ASSERT(tst,
         LISP_IS_OBJECT(&copy1));
  ASSERT(tst,
         LISP_IS_STRING(&copy1));
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&str), 2u);
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&copy1), 2u);
  ASSERT_EQ_PTR(tst,
                lisp_c_string(LISP_AS(&str, lisp_string_t)),
                lisp_c_string(LISP_AS(&copy1, lisp_string_t)));

  ASSERT_IS_OK(tst,
               lisp_copy_object(vm, &copy2, &copy1))
  ASSERT(tst,
         LISP_IS_OBJECT(&copy2));
  ASSERT(tst,
         LISP_IS_STRING(&copy2));
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&str), 3u);
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&copy1), 3u);
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&copy2), 3u);
  ...
  ASSERT_EQ_PTR(tst,
                lisp_c_string(LISP_AS(&str, lisp_string_t)),
                lisp_c_string(LISP_AS(&copy2, lisp_string_t)));
  ASSERT_EQ_CSTR(tst,
                 lisp_c_string(LISP_AS(&str, lisp_string_t)),
                 "abc");
  ASSERT_EQ_CSTR(tst,
...
static void test_make_substring_full(unit_test_t * tst) 
{
  ...
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&str), 1u);
  ASSERT_IS_OK(tst,
               lisp_make_substring(vm,
                                   &substr_full,
                                   LISP_AS(&str, lisp_string_t),
                                   0,
                                   10));
  ASSERT(tst,
         LISP_IS_OBJECT(&substr_full));
  ASSERT(tst,
         LISP_IS_STRING(&substr_full));
  ASSERT_EQ_PTR(tst,
                lisp_c_string(LISP_AS(&str, lisp_string_t)),
                lisp_c_string(LISP_AS(&substr_full, lisp_string_t)));
  ASSERT_EQ_U(tst,
              lisp_string_length(LISP_AS(&substr_full,
                                         lisp_string_t)), 10u);
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&str),
              1u);
...
static void test_make_substring_empty(unit_test_t * tst) 
{
  ...
  ASSERT_IS_OK(tst,
               lisp_make_substring(vm,
                                   ...
                                   10));
  ASSERT_EQ_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&substr_empty1,
                                               lisp_string_t), ""),
              0);
  ASSERT_EQ_U(tst,
              lisp_string_length(LISP_AS(&substr_empty1,
                                         lisp_string_t)),
              0u);
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&str),
  ...
  ASSERT_EQ_U(tst,
              ...
              1u);
  ASSERT_EQ_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&substr_empty2,
                                               lisp_string_t), ""),
              0);
  ASSERT_EQ_U(tst,
              lisp_string_length(LISP_AS(&substr_empty2,
                                         lisp_string_t)),
              0u);

  ASSERT_IS_OK(tst,
               lisp_unset_object(vm,
                                 &str));
  ASSERT_IS_OK(tst,
               lisp_unset_object(vm, &substr_empty1));
  ASSERT_IS_OK(tst,
               lisp_unset_object(vm, &substr_empty2));
...
static void test_make_substring_begin(unit_test_t * tst) 
{
  ...
  ASSERT_IS_OK(tst,
               lisp_make_substring(vm,
                                   ...
                                   4));
  ASSERT(tst,
         LISP_IS_OBJECT(&substr));
  ASSERT(tst,
         LISP_IS_STRING(&substr));
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&str), 1u);
  ASSERT_EQ_U(tst,
              LISP_REFCOUNT(&substr), 1u);
  ASSERT_EQ_PTR(tst,
                lisp_c_string(LISP_AS(&str, lisp_string_t)),
                lisp_c_string(LISP_AS(&substr, lisp_string_t)));
  ASSERT_EQ_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&substr,
                                               lisp_string_t),
                                       "0123"), 0);
  ASSERT_LT_I(tst,
              lisp_string_cmp_c_string(LISP_AS(&substr,
