...
static void test_push_halt(unit_test_t * tst) 
{
  lisp_unit_context_t * ctx = lisp_create_unit_context(&lisp_vm_default_param,
                                                       tst);
  size_t i;
  for(i = 0; i < ctx->env->max_call_stack_size; i++) 
  {
    ASSERT_IS_OK(tst, lisp_push_halt(ctx->env));
  }
  ASSERT_IS_STACK_OVERFLOW(tst, lisp_push_halt(ctx->env));
  lisp_free_unit_context(ctx);
}

static void test_push_halt_alloc_error(unit_test_t * tst) 
{
  lisp_unit_context_t * ctx = lisp_create_unit_context(&lisp_vm_default_param,
                                                       tst);
  memcheck_expected_alloc(0);
  ASSERT_IS_ALLOC_ERROR(tst, lisp_push_halt(ctx->env));
...
void test_eval(unit_context_t * ctx)
{
  ...
  TEST(suite, test_push_halt);
  TEST(suite, test_push_halt_alloc_error);
}
